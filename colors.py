...
def multidist(i1, i2):
    ...
    Args:
        c1 (list or numpy.ndarray): first/starting RGBF vector with the following structure:
            [red (of most dominant color), blue, green, frequency, red (of 2nd most dominant color), blue, green, frequency, ...]
        c2 (list or numpy.ndarray): second/end RGBF vector with the following structure:
            [red (of most dominant color), blue, green, frequency, red (of 2nd most dominant color), blue, green, frequency, ...]
    ...
    for j, rgbf1 in enumerate(i1): # i also tried a rank wise weighting using j and k but it didn't work as well
        for k, rgbf2 in enumerate(i2):
            f1 = rgbf1[-1]
            f2 = rgbf2[-1]
            distance += labdist(rgbf1[:3], rgbf2[:3]) * np.sqrt(f1 * f2) # weight by geometric mean of the 2 frequencies
            tf += np.sqrt(f1 * f2)
    distance /= tf # normalize
    return distance
